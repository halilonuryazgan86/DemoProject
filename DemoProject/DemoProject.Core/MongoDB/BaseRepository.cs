...
namespace DemoProject.Core.MongoDB
{
    ...
    public class BaseRepository<T> : IBaseRepository<T> where T : BaseCollection, new()
    {
        ...
        public BaseRepository(MongoDBConnectionSetting mongoDBConnectionSetting)
        {
            ...
            try
            {
                var client = new MongoClient($"mongodb+srv://{_mongoDBConnection.UserName}:{_mongoDBConnection.Password}@cluster0.tud88kd.mongodb.net/{_mongoDBConnection.DBName}?retryWrites=true&w=majority");
        ...
        public async Task<List<T>> GetListByFilterDefinitionAsync(FilterDefinition<T> filter)
        {
